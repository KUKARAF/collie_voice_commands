...
use crate::collie::{find_focused_pane, AgentStatus, CollieClient};
...
#[tauri::command]
pub async fn send_command(app: AppHandle, text: String) -> Result<SendCommandResult, String> {
    ...
    let pane_id = find_focused_pane(&snapshot)
        .ok_or("no focused pane — open one in the Collie UI first")?
        .pane_id
        .clone();
    ...
    let action = match resolved.keys.filter(|k| !k.is_empty()) {
        Some(keys) => collie.keys(&pane_id, keys).await,
        None => collie.reply(&pane_id, &resolved.reply).await,
    }
    .map_err(|e| e.to_string())?;
    if !action.ok {
        return Err(action
